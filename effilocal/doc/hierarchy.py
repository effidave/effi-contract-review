...
def infer_block_hierarchy(blocks: Iterable[MutableMapping[str, Any]]) -> None:
    ...
    def _resolve_parent_from_ordinals(index: int, group_id: Optional[str]) -> Optional[str]:
        ...
        for source_map in (
            recent_ordinals_by_group.get(group_id),
            recent_ordinals_global,
        ):
            if not source_map:
                continue
        ...
        for candidate_ctx in preceding_chain:
            ...
            candidate_ord_tuple = _ordinal_to_tuple(ordinal_text)
            if candidate_ord_tuple is None:
                continue
            expected_next = candidate_ord_tuple[:-1] + (candidate_ord_tuple[-1] + 1,)
            matching_ctx = None
            for successor in succeeding_chain:
                ...
                if (
                    ...
                    matching_ctx = successor
                    break
            if matching_ctx is not None:
                chosen_ctx = candidate_ctx
                break

        if chosen_ctx is not None:
            return chosen_ctx.block_id
        if prev_any is not None:
            if fallback_level0_ctx is not None:
                return fallback_level0_ctx.block_id
            return prev_any.block_id
        return None
    ...
    for index, block in enumerate(block_list):
        ...
        def _finalise(
            ...
            list_payload: Optional[MutableMapping[str, Any]],
        ) -> None:
            ...
            if isinstance(list_payload, MutableMapping):
                ...
                fmt = str(list_payload.get("format") or "").lower()
                if fmt == "none":
                    resolved_parent = _resolve_parent_from_ordinals(index, group_id)
                ...
                if fmt == "none":
                    ...
                    if parent_id is not None:
                        parent_clause_id = (
                            by_id.get(parent_id, {}).get("clause_group_id") or parent_id
                        )
                    block["clause_group_id"] = (
                        parent_clause_id or block.get("clause_group_id") or block_id
                    )
                    return
                if level == 0:
                    root_id = block_id
                    ...
                    parent_clause_id = (
                        by_id.get(parent_id, {}).get("clause_group_id") if parent_id else None
                    )
                    block["clause_group_id"] = (
                        parent_clause_id or block.get("clause_group_id") or block_id
                    )
            ...
            if not assigned:
                group_id_local = plain_runs.get(parent_id)
                if group_id_local is None:
                    group_id_local = block_id
                    plain_runs[parent_id] = group_id_local
                block["clause_group_id"] = group_id_local
        ...
        group_id_candidate = block.get("restart_group_id")
        if block_type == "paragraph":
            if group_id_candidate is None and list_stack:
                group_id_candidate = list_stack[-1].group_id
            if group_id_candidate is not None:
                resolved_parent = _resolve_parent_from_ordinals(index, group_id_candidate)
        if resolved_parent is not None:
            parent_id = resolved_parent
