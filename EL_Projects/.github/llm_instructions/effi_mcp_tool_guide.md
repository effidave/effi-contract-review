...
- `save_document_as_markdown` – save document content as Markdown file
...
**Para ID Tools (direct paragraph manipulation):**
- `get_text_by_para_id` – get text content of a paragraph by its native w14:paraId
- `replace_text_by_para_id` – replace entire text content of a paragraph by its paraId

**Attachments (Schedules, Annexes, Exhibits):**
- `add_paragraph_after_attachment` – insert paragraph after specific attachment
- `add_paragraphs_after_attachment` – bulk insert paragraphs after attachment
- `add_new_attachment_after` – add new attachment (Schedule, Annex, Exhibit) after existing one

**Footnotes:**
- `add_footnote_to_document` – add footnote to a specific paragraph
- `add_footnote_after_text` – add footnote after specific text with superscript formatting
- `customize_footnote_style` – customize footnote numbering and formatting
- `delete_footnote_from_document` – delete a footnote from document

**Protection:**
- `protect_document` – add password protection to a document
- `unprotect_document` – remove password protection from a document

**Numbering Analysis:**
- `analyze_document_numbering` – analyze numbering structure using NumberingInspector
- `get_numbering_summary` – get high-level summary of numbering styles
- `extract_outline_structure` – extract document outline based on numbering
...
**Work Plan Management (Task Tracking):**
- `get_work_plan` – get current work plan with tasks, documents, and stats
- `add_task` – add new task to plan (position: 'start', 'end', or 'at' with ordinal)
- `update_task` – update task's title, description, or status
- `delete_task` – remove task from plan
- `move_task` – move task to new position (0-based ordinal)
- `start_task` – mark task as in_progress
- `complete_task` – mark task as completed
- `block_task` – mark task as blocked
- `add_plan_document` – add document to plan's tracked documents
- `remove_plan_document` – remove document from plan
- `list_plan_documents` – list all tracked documents

**See [Work Plan Tools Guide](work_plan_tools.md) for detailed task management patterns.**
...
| "get paragraph by ID" | `get_text_by_para_id()` | Retrieve paragraph by paraId |
| "replace paragraph content" | `replace_text_by_para_id()` | Replace paragraph text by paraId |
| "add a schedule after X" | `add_new_attachment_after()` | Insert new attachment |
| "show my tasks" | `get_work_plan()` | Get work plan with tasks |
| "add a task" | `add_task()` | Add task to work plan |
| "start this task" | `start_task()` | Mark task as in_progress |
| "complete this task" | `complete_task()` | Mark task as completed |
| "track this document" | `add_plan_document()` | Add document to plan |
...
EL_Projects\[project name]\
  ...
  ├─ plans\                      # Work plan files
  │   └─ current\                # Active plans
  │       ├─ plan.md             # Plan tasks in Markdown with YAML frontmatter
  │       └─ plan.meta.json      # Plan metadata (tracked documents)
...
**Situation:** User wants to track and manage tasks for document review

**Workflow:**
1. Use `get_work_plan()` to see current tasks
2. Use `add_task()` to add new tasks
3. Use `start_task()` before working on a task
4. Use `complete_task()` when done
5. Use `add_plan_document()` to track documents being reviewed

**Example:**
```
User: "Show me my current tasks for this project"

Agent executes:
- get_work_plan(filename="contract.docx")
  → Returns: {"tasks": [...], "documents": [...], "stats": {...}}

Agent reports:
📋 Work Plan for Norton - R&D:
  1. [pending] Review indemnification clause
  2. [in_progress] Add confidentiality terms
  3. [completed] Format Schedule 1

Stats: 1 pending, 1 in progress, 1 completed
```

**Adding and Starting a Task:**
```
User: "Add a task to review the warranty section, and start it"

Agent executes:
- add_task(
    filename="contract.docx",
    title="Review warranty section",
    description="Check warranty terms align with business requirements"
  )
  → Returns task_id: "task_abc123"
- start_task(filename="contract.docx", task_id="task_abc123")

Agent reports:
✅ Added task: "Review warranty section"
✅ Started task (status: in_progress)
```

**See [Work Plan Tools Guide](work_plan_tools.md) for complete task management patterns.**
...
| Work Plans | Use Plan tools for task tracking (`plans\current\`) |
...
- **work_plan_tools.md** – Complete guide to task management tools
