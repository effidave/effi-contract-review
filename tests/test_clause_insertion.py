...
async def test_add_paragraph_after_nested_clause(temp_numbered_doc):
    """Test adding a paragraph after a nested clause like 1.1."""
    result = await add_paragraph_after_clause(
        str(temp_numbered_doc),
        "1.1",
        "New content under 1.1",
        inherit_numbering=True
    )
    
    assert "Paragraph added after clause '1.1'" in result
    
    # Verify
    doc = Document(str(temp_numbered_doc))
    found = False
    for para in doc.paragraphs:
        if "New content under 1.1" in para.text:
            found = True
            break
    assert found
...
async def test_add_paragraph_after_deep_nested_clause(temp_numbered_doc):
    """Test adding a paragraph after a deeply nested clause like 1.1.1."""
    result = await add_paragraph_after_clause(
        str(temp_numbered_doc),
        "1.1.1",
    ...
    assert "Paragraph added after clause '1.1.1'" in result
