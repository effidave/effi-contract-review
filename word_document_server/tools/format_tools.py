...
async def set_background_highlight(
    ...
    Returns a human-readable status string (consistent with other tools).
    """
    filename = ensure_docx_extension(filename)
    ...
    try:
        ...
        if use_shading:
            ...
            if len(fill) != 6 or any(c not in "0123456789ABCDEF" for c in fill):
                return f"Invalid hex color for shading: {color}"
            # Clear any palette highlight just in case
            run_target.font.highlight_color = None
            _set_run_shading(run_target, fill)
            mode_desc = f"shading fill={fill}"
        else:
            # Word highlight palette (e.g., YELLOW, LIGHT_GRAY)
            name_u = color.upper().replace(" ", "_")
            value = getattr(WD_COLOR_INDEX, name_u, None)
            run_target.font.highlight_color = value
            # Clear shading if present
            _set_run_shading(run_target, None)
            mode_desc = f"highlight={color}"

        doc.save(filename)
        return f"Background applied to paragraph {paragraph_index} [{start_pos}:{end_pos}] using {mode_desc}."
    except Exception as e:
        return f"Failed to apply background: {str(e)}"

def _read_run_styles(run) -> Dict[str, Optional[str]]:
    """
    Inspect a run's rPr and return color/highlight/shading in a JSON-friendly way.
    - color: run text color hex (RRGGBB) if set via <w:color w:val="...">
    - highlight: Word highlight palette name (raw 'val') if present
    - shading_fill: hex fill from <w:shd w:fill="..."> if present
    """
    styles = {"color": None, "highlight": None, "shading_fill": None}
    rPr = getattr(run._element, "rPr", None)
    if rPr is None:
        return styles

    # <w:color w:val="RRGGBB">
    color_el = rPr.find(qn("w:color"))
    if color_el is not None:
        val = color_el.get(qn("w:val")) or color_el.get("w:val")
        if val and val.lower() not in {"auto"}:
            styles["color"] = val.upper()

    # <w:highlight w:val="yellow"> (Word palette)
    hi_el = rPr.find(qn("w:highlight"))
    if hi_el is not None:
        val = hi_el.get(qn("w:val")) or hi_el.get("w:val")
        if val:
    ...
    if shd_el is not None:
        fill = shd_el.get(qn("w:fill")) or shd_el.get("w:fill")
        if fill:
            styles["shading_fill"] = fill.upper()
...
def _paragraph_runs_snapshot(paragraph) -> List[Dict[str, Optional[str]]]:
    """
    Return a list of {text, color, highlight, shading_fill, bold, italic, underline}
    for each run in a paragraph, for easy verification in chat.
    """
    result = []
    for run in paragraph.runs:
        s = _read_run_styles(run)
        result.append({
            "text": run.text,
            "color": s["color"],
            "highlight": s["highlight"],
            "shading_fill": s["shading_fill"],
            "bold": bool(run.bold),
...
def get_document_runs(filename: str, paragraph_index: int) -> Dict:
    """
    Debug/verify helper: return run-level text and styles for a paragraph.
    """
    try:
        doc = Document(filename)
    except Exception as e:
        return {"ok": False, "message": f"Unable to open document: {e}"}

    if paragraph_index < 0 or paragraph_index >= len(doc.paragraphs):
        return {"ok": False, "message": f"paragraph_index out of range (0..{len(doc.paragraphs)-1})"}
    ...
    return {
        ...
        "text": p.text,
        "runs": _paragraph_runs_snapshot(p),
